...
- `public-config.js` の正規GAS URLは公開接続先であり秘密値ではない。入力可能な任意URLへ戻さない。
- AI由来データを `innerHTML` へ直接入力しない。固定の許可値へ正規化し、表示は `textContent` を優先する。
- HTMLのCSPから `unsafe-inline` / `unsafe-eval` を追加しない。インラインscriptやstyleが必要になった場合は外部ファイルへ分離する。
- 端末鍵の登録・再接続は、Mac側の指紋読戻しと監査記録を確認してから切り替える。失効応答が曖昧なときは端末内の鍵を先に消さない。
- 現在入力と写真付き一時保存は30日で自動整理する。保持期間を変更する場合はUI・README・テストも同時に更新する。
- 修正前に `scripts/create_backup.sh`、修正後に `scripts/verify_backup.sh <backup>` を実行し、バックアップから全テストを実行できることまで確認する。
